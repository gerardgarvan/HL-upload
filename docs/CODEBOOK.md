...
| `PAYER_CATEGORY_AT_FIRST_DX` | String | **Mode** of **valid** payer category among encounters’s first HL diagnosis date, within **±30 days**. **Mode** (most frequent) **valid** (non-missing) payer category in that window; only valid effective payer is counted. Null if no HL diagnosis, no encounter in window, or no valid payer in window. |
| `PAYER_CATEGORY_AT_FIRST_CHEMO` | String | **Mode** of **valid** payer among encounters’s **first chemo date**, within **±30 days**. **Mode** of **valid** payer in window; only non-missing counts. Null if no chemo date or no valid payer in window. |
| `PAYER_CATEGORY_AT_LAST_CHEMO` | String | **Mode** of **valid** payer among encounters’s **last chemo date**, within **±30 days**. **Mode** of **valid** payer in window; only non-missing counts. Null if no chemo or no valid payer in window. |
...
| `PAYER_CATEGORY_AT_FIRST_RADIATION` | String | **Mode** of **valid** payer within **±30 days** of first radiation date (TR DT_RAD or PROCEDURES radiation CPTs). Null if no radiation or no valid payer in window. |
| `PAYER_CATEGORY_AT_LAST_RADIATION` | String | **Mode** of **valid** payer within **±30 days** of last radiation date. Null if no radiation or no valid payer in window. |
| `PAYER_CATEGORY_AT_FIRST_SCT` | String | **Mode** of **valid** payer within **±30 days** of first SCT procedure date (PROCEDURES SCT CPTs). Null if no SCT or no valid payer in window. |
| `PAYER_CATEGORY_AT_LAST_SCT` | String | **Mode** of **valid** payer within **±30 days** of last SCT procedure date. Null if no SCT or no valid payer in window. |
...
| `HAD_CHEMO` | Int8 | 1 if the patient has at least one chemo date; 0 otherwise. Used for treatment-specific summary (chemo cohort). |
| `HAD_RADIATION` | Int8 | 1 if the patient has at least one radiation date; 0 otherwise. Used for treatment-specific summary (radiation cohort). |
| `HAD_SCT` | Int8 | 1 if the patient has at least one SCT procedure date; 0 otherwise. Used for treatment-specific summary (SCT cohort). |
